...
describe Spree::Product, :type => :model do
  ...
  context "#images" do
    ...
    let(:params) { {:variant_ids => [product.master.id], :attachment => image, :alt => "position 2", :position => 2} }

    before do
      Spree::Image.create(params)
      Spree::Image.create(params.merge({:alt => "position 1", :position => 1}))
      Spree::Image.create(params.merge({:variant_ids => other_product.master.id, :alt => "position 1", :position => 2}))
    end
